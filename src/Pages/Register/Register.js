    ...
    return (
        <div style={{
            ...
            <div className="main-content-wrapper">
                ...
                <form className='input-form' onSubmit={e => {e.preventDefault(); document.getElementById('password-input').value === document.getElementById('confirm-password-input').value ? createAccount( e, document.getElementById('email-input').value, document.getElementById('name-input').value, document.getElementById('password-input').value, document.getElementById('company-code-input') ) : setLoading(['error', 'Password and Confirm Password do not match!'])}}>
                    <div className="input-wrapper">
                        ...
                        <br />
                        <br />
                        <div className='label-text' style={{ width: '14em', marginLeft: '50%', transform: 'translateX(-50%)' }}>COMPANY CODE (optional)</div>
                        <input type="password" className='input-box' id='company-code-input' style={{ width: '8em', marginLeft: '50%', transform: 'translateX(-50%)' }} />
                        <input type="submit" className='input-button' value='Register' />
                    </div>
                </form>
                <div className="fine-print-text" style={{marginTop: 420}}><a className='link-text' href='/login'>Already have an account?</a></div>
            </div>
        </div>
    );

    async function createAccount(event, email, name, password, company_code_input){
